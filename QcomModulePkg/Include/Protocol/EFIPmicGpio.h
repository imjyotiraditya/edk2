...
typedef enum
{
  ...
  EFI_PM_GPIO_EXT_PIN_ENABLE,   /**< Puts EXT_PIN at high Z state and disablesthe block. */
...
typedef enum
{
  EFI_PM_GPIO_INPUT_LOW,              /**< GPIO input low. */
  EFI_PM_GPIO_INPUT_HIGH,             /**< GPIO input high. */
}EFI_PM_GPIO_STATUS_VALUE_TYPE;
...
typedef struct 
{
    ...
    EFI_PM_GPIO_EXT_PIN_CONFIG_TYPE      GpioExtPinConfig;         /**< GPIO EXT_PIN configuration. */
    EFI_PM_GPIO_STATUS_VALUE_TYPE        GpioStatusValue;          /**< GPIO status value. */
...
/** @ingroup efi_pmicGpio_digital_output
  ...
  @param[in]  Gpio               Which GPIO; see # EFI_PM_GPIO_PERIPH_INDEX for 
                                 details.
  @param[in]  GpioPullSelType    Select current source pulls type; see 
                                 #EFI_PM_GPIO_I_SRC_PULL_TYPE for 
                                 details.  
...
EFI_STATUS (EFIAPI *EFI_PM_GPIO_SET_PULL_SEL)(
  IN UINT32                             PmicDeviceIndex,
  IN EFI_PM_GPIO_PERIPH_INDEX           Gpio,
  IN EFI_PM_GPIO_I_SRC_PULL_TYPE        GpioPullSelType
);


/* EFI_PM_GPIO_GET_PMIC_REG */ 
typedef
EFI_STATUS(EFIAPI *EFI_PM_GPIO_GET_PMIC_REG)
(
 IN UINT32                             PmicDeviceIndex,
...
/* EFI_PM_GPIO_WRITE_PMIC_REG */ 
typedef
EFI_STATUS(EFIAPI *EFI_PM_GPIO_WRITE_PMIC_REG)
(
 IN UINT32                             PmicDeviceIndex,
 IN UINT32                             RegAddress,
 IN UINT32                             RegValue
...
struct _EFI_QCOM_PMIC_GPIO_PROTOCOL {
  ...
  EFI_PM_GPIO_GET_PMIC_REG              GetPmicReg;
  EFI_PM_GPIO_WRITE_PMIC_REG            WritePmicReg;
  };
